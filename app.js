...
function initializeApp() {
    ...
    if (savedUser) {
        ...
        console.log('No user found, showing welcome page');
    ...
    if (savedPetitions) {
        ...
        console.log('Loaded petitions:', petitions);
...
function setupEventListeners() {
    ...
    const showRegBtn = document.getElementById('showRegistration');
    if (showRegBtn) {
        showRegBtn.addEventListener('click', showRegistration);
    }
    ...
    if (petitionForm) {
        petitionForm.addEventListener('submit', handlePetitionSubmission);
    }
    ...
    const createPetitionBtn = document.getElementById('createPetitionBtn');
    if (createPetitionBtn) {
        createPetitionBtn.addEventListener('click', function() {
            showSection('create');
        });
    }
...
function updatePetitionLimitInfo() {
    ...
    if (sectionHeader) {
        let limitInfoElement = sectionHeader.querySelector('.limit-info');
        if (!limitInfoElement) {
            limitInfoElement = document.createElement('div');
...
function handlePetitionSubmission(e) {
    e.preventDefault();
    console.log('Handling petition submission...');
...
function updateMyPetitionsList() {
    console.log('Updating my petitions list...');
...
function updateLocalPetitionsList() {
    console.log('Updating local petitions list...');
    const container = document.getElementById('localPetitionsList');
    
    if (!container) {
        console.log('Local petitions container not found');
        return;
    }
...
function showSuccessModal(petitionId) {
    ...
    const petitionIdElement = document.getElementById('petitionId');
    
    if (petitionIdElement) {
        petitionIdElement.textContent = `#${petitionId}`;
    }
...
function loadSampleData() {
    if (petitions.length === 0) {
        ...
        petitionCounter = 4;
        localStorage.setItem('petitions', JSON.stringify(petitions));
        console.log('Sample data loaded:', petitions);
    }
